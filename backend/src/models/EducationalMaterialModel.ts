...
export class EducationalMaterialModel {
  static async getAll() {
    return await sql`SELECT * FROM "EducationalMaterial" ORDER BY id DESC`;
  }

  static async getById(id: number) {
    const res = await sql`SELECT * FROM "EducationalMaterial" WHERE id = ${id}`;
    return res[0] || null;
  }
  ...
  static async delete(id: number) {
    await sql`DELETE FROM "EducationalMaterial" WHERE id = ${id}`;
    return { success: true };
  }
}
