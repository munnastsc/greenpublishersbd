...
export class PublisherModel {
  static async getAll() {
    return await sql`SELECT * FROM "Publisher" ORDER BY id DESC`;
  }

  static async getById(id: number) {
    const res = await sql`SELECT * FROM "Publisher" WHERE id = ${id}`;
    return res[0] || null;
  }
  ...
  static async delete(id: number) {
    await sql`DELETE FROM "Publisher" WHERE id = ${id}`;
    return { success: true };
  }
}
