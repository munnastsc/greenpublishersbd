...
export class HomeSectionModel {
  static async getAll() {
    return await sql`SELECT * FROM "HomeSection" ORDER BY id DESC`;
  }

  static async getById(id: number) {
    const res = await sql`SELECT * FROM "HomeSection" WHERE id = ${id}`;
    return res[0] || null;
  }
  ...
  static async delete(id: number) {
    await sql`DELETE FROM "HomeSection" WHERE id = ${id}`;
    return { success: true };
  }
}
