...
class PublisherModel {
    static async getAll() {
        return await (0, db_1.sql) `SELECT * FROM "Publisher" ORDER BY id DESC`;
    }
    static async getById(id) {
        const res = await (0, db_1.sql) `SELECT * FROM "Publisher" WHERE id = ${id}`;
        return res[0] || null;
    }
    ...
    static async delete(id) {
        await (0, db_1.sql) `DELETE FROM "Publisher" WHERE id = ${id}`;
        return { success: true };
    }
}
