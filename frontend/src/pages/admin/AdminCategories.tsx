...
export default function AdminCategories() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/categories', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    if (res.ok) {
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/categories', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    fetchItems();
  };
